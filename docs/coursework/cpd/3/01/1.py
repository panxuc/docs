# 第一次作业：
# 买车问题
# 假设用户要买一辆汽车，总价30000元，用户将按照0元首付及每月
# 等额分期付款的方式进行购买，年贷款利率为7%，计划60个月还清，请计算用
# 户每个月需要还银行贷款的数额。
# 具体要求：
# 	①从键盘输入各项参数，以回车键或空格键作为输入结束标志。最后一个输
# 	入结束后，程序自动显示计算结果。
# 	②输入和输出要有适当的文字提示。
# 	③附加任务（供参考选做，不加分）：处理带有汉字的输入内容；从文件导
# 	入多个用户的状况进行批量计算并将结果保存到新文件；计算结果非整
# 	年时，将结果精度近似到月份。
